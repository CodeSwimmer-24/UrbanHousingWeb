...
export const ACCENT_MUTED = '#EFF6FF';
...
/** Same 5-step tracker as publish flow — last chip is Review instead of Pay / Plan when editing */
export const STEP_META_EDIT = [
  STEP_META[0],
  STEP_META[1],
  STEP_META[2],
  STEP_META[3],
...
/* ─── Step Progress Bar ─────────────────────────────────────────────────────── */
export function ProgressBar({ step, stepMeta = STEP_META }) {
  const meta = stepMeta ?? STEP_META;
  const denom = Math.max(meta.length - 1, 1);
  const pct = ((step - 1) / denom) * 100;
  return (
    <View style={pb.wrap}>
      <View style={pb.track}>
        ...
        {meta.map((s, i) => {
          ...
          return (
            <View key={i} style={[pb.dot, { left: `${(i / denom) * 100}%` }, (done || active) && pb.dotActive, done && pb.dotDone]}>
              {done
...
/* ─── Reusable primitives ───────────────────────────────────────────────────── */
export function Section({ title, subtitle, children, last }) {
  return (
...
export function Field({ label, required, style, ...props }) {
...
/* ─── Step 1 ────────────────────────────────────────────────────────────────── */
export function Step1({ role, setRole, listingType, setListingType, propertyType, setPropertyType }) {
  return (
    <View>
...
/* ─── Step 2 ────────────────────────────────────────────────────────────────── */
export function Step2({ ownerName, setOwnerName, mobile, setMobile, whatsapp, setWhatsapp, email, setEmail, currentAddress, setCurrentAddress }) {
  return (
    <View>
...
/* ─── Step 4 ────────────────────────────────────────────────────────────────── */
export function Step4({ videoCount, setVideoCount, imageCount, setImageCount }) {
  return (
    <View>
...
/* ─── Step 5 ────────────────────────────────────────────────────────────────── */
export function Step5ChoosePlan({ plan, setPlan }) {
